...
import { selectExercises, getAllModalExercises, getModalOptions } from "../../lib/exercise-logic";
import { recordAnswer, recordSession, recordQuestionAnswer } from "../../lib/stats";
import { ExercisePhase } from "../../lib/types";
import type { ModalExerciseData } from "../../data/modal-exercises";
...
const exerciseImage = require("../../assets/images/modals-card.png") as ImageSourcePropType;

export default function ModalsScreen() {
  const router = useRouter();
  const [phase, setPhase] = useState<ExercisePhase>("setup");
  const [targetCount, setTargetCount] = useState(10);
  const [exercise, setExercise] = useState<ModalExerciseData>(
    () => getAllModalExercises()[0]
  );
  const [options, setOptions] = useState<string[]>(() =>
    getModalOptions(exercise.modalVerb, exercise.person, exercise.correctForm)
  );
  ...
  const exercisesRef = useRef<ModalExerciseData[]>([]);
  ...
  const handleStart = useCallback(async (count: number) => {
    ...
    totalRef.current = 0;
    correctRef.current = 0;
    const { exercises, questionIds } = await selectExercises("modals", getAllModalExercises(), count);
    exercisesRef.current = exercises;
    questionIdsRef.current = questionIds;
    exerciseIndexRef.current = 0;
    setExercise(exercises[0]);
    setOptions(getModalOptions(exercises[0].modalVerb, exercises[0].person, exercises[0].correctForm));
    setSelected(null);
    setIsCorrect(null);
  ...
  const handleNext = useCallback(() => {
    ...
    Animated.timing(fadeAnim, {
      ...
      setExercise(ex);
      setOptions(getModalOptions(ex.modalVerb, ex.person, ex.correctForm));
      setSelected(null);
      setIsCorrect(null);
      setShowCelebration(false);
  ...
  if (phase === "setup") {
    return (
      <ExerciseSetup
        title="Modal Verbs"
        subtitle="Practice können, müssen, dürfen, sollen, wollen, mögen"
        image={exerciseImage}
          ...
          <View style={styles.feedbackRow}>
            {isCorrect ? (
              ...
              <Text style={styles.wrongText}>
                Richtig: <Text style={styles.endingHighlight}>{exercise.correctForm}</Text>
              </Text>
            )}
