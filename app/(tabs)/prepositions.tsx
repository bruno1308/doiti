...
import { selectExercises, getAllPrepositionExercises, getPrepositionOptions } from "../../lib/exercise-logic";
...
export default function PrepositionsScreen() {
  ...
  const [exercise, setExercise] = useState<PrepositionExerciseData>(
    () => getAllPrepositionExercises()[0]
  );
  const [options, setOptions] = useState<string[]>(() =>
    getPrepositionOptions(exercise.preposition)
  );
  ...
  const exercisesRef = useRef<PrepositionExerciseData[]>([]);
  ...
  const handleStart = useCallback(async (count: number) => {
    ...
    totalRef.current = 0;
    correctRef.current = 0;
    const { exercises, questionIds } = await selectExercises("prepositions", getAllPrepositionExercises(), count);
    exercisesRef.current = exercises;
    questionIdsRef.current = questionIds;
    exerciseIndexRef.current = 0;
    setExercise(exercises[0]);
    setOptions(getPrepositionOptions(exercises[0].preposition));
    setSelected(null);
    setIsCorrect(null);
  ...
  const handleSelect = useCallback((option: string) => {
    ...
    const result = option === exercise.preposition;
  ...
  const handleNext = useCallback(() => {
    ...
    Animated.timing(fadeAnim, {
      ...
      setExercise(ex);
      setOptions(getPrepositionOptions(ex.preposition));
      setSelected(null);
      setIsCorrect(null);
      setShowCelebration(false);
  ...
  if (phase === "setup") {
    return (
      <ExerciseSetup
        title="Prepositions"
        subtitle="Practice German prepositions and their cases"
        image={exerciseImage}
  ...
  const getOptionStyle = (option: string) => {
    ...
    if (option === exercise.preposition) return styles.optionReveal;
  ...
  const getOptionTextStyle = (option: string) => {
    ...
    if (option === exercise.preposition) return styles.optionTextReveal;
          ...
          <View style={styles.feedbackRow}>
            {isCorrect ? (
              ...
              <Text style={styles.wrongText}>
                Richtig: <Text style={styles.endingHighlight}>{exercise.preposition}</Text>
